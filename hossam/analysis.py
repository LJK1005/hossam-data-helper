...
def my_normal_test(data: DataFrame, method: str = "n") -> None:
    ...
    for c in data.columns:
        ...
        print(
            "[{0}-{1}] statistic: {2:.3f}, p-value: {3:.3f}, 정규성 충족 여부: {4}".format(
                n, c, s, p, p > 0.05
            )
        )
...
def my_equal_var_test(data: DataFrame, normal_dist: bool = True) -> None:
    ...
    Args:
        data (DataFrame): 데이터프레임 객체
        normal_dist (bool, optional): True일 경우 정규분포를 따르는 데이터에 대한 등분산성 검정을 수행한다. Defaults to True.
    """
    # 데이터프레임의 각 필드를 리스트로 묶음
    # -> fields = [ data['a'], data['b'], data['c'] ]
    fields: list = list(map(lambda x: data[x], data.columns))
    ...
    print(
        "{0} 검정: statistic: {1:.3f}, p-value: {2:.3f}, 등분산성 충족 여부: {3}".format(
            n, s, p, p > 0.05
        )
    )
...
def my_normal_equal_var_2field(
    ...
    Args:
        data (DataFrame): 데이터프레임 객체
        xname (str, optional): 종속변수의 컬럼명. Defaults to 'y'.
...
def my_anova(
    ...
    Args:
        ...
        hue (any): 명목형 변수의 컬럼명을 저장하고 있는 리스트
        equal_var (bool, optional): 등분산성 가정 여부. Defaults to True.
        post (bool, optional): 사후검정 수행 여부. Defaults to False.
    """
    ...
    print(
        "[anova_lm] statistic: {0:.3f}, p-value: {1:.3f}, {2}가설 채택".format(
        ...
        if equal_var:
            ...
            # 샘플수가 다른 경우 본페로니 방법
                ...
                result = pairwise_tests(
                    data=data, dv=target, between=hue, padjust="bonf"
                )
...
def my_correlation(
    ...
    if heatmap:
        my_heatmap(
            data.corr(method="pearson" if method == "p" else "spearman"),
            figsize=figsize,
            dpi=dpi,
        )
    else:
